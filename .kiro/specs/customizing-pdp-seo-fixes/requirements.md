...
- **ID_Leak_Substring**: The contiguous substring matching the regular expression `\s-\s\d{2,}\s*$` — a space-hyphen-space prefix, followed by two or more digits, optional trailing whitespace, anchored to end-of-string. Used only to *identify and count* contaminated rows; this revision no longer relies on stripping it, because the title is fully reconstructed (R7).
- **Title_Builder**: The pure function `buildCakeTitle(input, budget?)` exported from `src/lib/seo/cakeTitle.ts`. Given a design's structured attributes (keyword, cakeType, icing colors, color_type, tags, hero-topper descriptions) it returns a customer-facing title string WITHOUT the ` | Genie.ph` suffix. Deterministic, no I/O, no React imports.
- **Title_Structure**: The canonical segment order emitted by Title_Builder: `[Theme] [Color] [Detail] [Type] [Occasion] Cake`, where any segment that is empty, redundant with an earlier segment, or trimmed for budget is omitted, and the head always terminates in the literal word `Cake` (de-duplicated so the string never contains `Cake Cake`).
- **Theme_Segment**: The leading segment derived from `design.keyword` (title-cased, trailing literal `Cake` stripped). When `design.keyword` is empty/whitespace, Theme_Segment is the literal `Custom`.
- **Inspired_Qualifier**: The literal suffix `-Inspired` appended to Theme_Segment when, and only when, `design.keyword` matches (case-insensitive, token-boundary) any entry in the Franchise_List.
- **Franchise_List**: The curated list of entertainment/character/franchise IP tokens in `src/lib/seo/cakeTitle.ts`. Real-company brand names that depict a brand rather than a licensable character (McDonald's, Jollibee, Red Horse, Shopee, etc.) are intentionally EXCLUDED.
- **PII_Source_Fields**: Fields that contain customer-supplied personal data and MUST NOT feed any title — `analysis_json.cake_messages[].text` (real names/greetings) and the contaminated `seo_title` being replaced. Title_Builder reads only non-PII structured attributes.
- **Reconstructed_Title**: The Title_Builder output stored into `cakegenie_analysis_cache.seo_title` by both the write path (R10) and the backfill (R7), replacing the prior AI-emitted or ID-leaked value.
- **Backup_Store**: A dedicated table `cakegenie_analysis_cache_seo_title_backup(slug TEXT, seo_title_before TEXT, backed_up_at TIMESTAMPTZ, migration_id TEXT)`. Every row whose `seo_title` is changed by the backfill has its prior value written here first. Retained for at least 30 days post-apply.
...
### Requirement 6: Deterministic Reconstructed Title Within Budget

**User Story:** As a Google search user scanning results, I want each cake PDP title to clearly and specifically describe that exact design (theme, colour, key detail, occasion) within the visible character cap, so that I can tell the listings apart and click the one I want.

#### Acceptance Criteria

1. THE Title_Builder SHALL be a pure function `buildCakeTitle(input, budget?)` exported from `src/lib/seo/cakeTitle.ts` with `budget` defaulting to `49` (Title_Budget); it SHALL perform no I/O and import no React.
2. THE Title_Builder SHALL emit a string conforming to Title_Structure `[Theme] [Color] [Detail] [Type] [Occasion] Cake`, always terminating in the literal word `Cake` and NEVER containing the substring `Cake Cake` (case-insensitive).
3. THE Title_Builder SHALL derive Theme_Segment from `input.keyword` title-cased with any trailing literal `Cake` removed; WHEN `input.keyword` is null, undefined, empty, or whitespace-only, THE Title_Builder SHALL use the literal `Custom` as Theme_Segment.
4. WHEN `input.keyword` matches any Franchise_List token (case-insensitive, token-boundary), THE Title_Builder SHALL append the Inspired_Qualifier `-Inspired` to Theme_Segment exactly once; WHEN it does not match, THE Title_Builder SHALL NOT append `-Inspired`.
5. THE Title_Builder SHALL NOT read, include, or derive any title text from PII_Source_Fields (`cake_messages[].text` or the prior `seo_title`).
6. WHEN `input.colorType` equals `multicolor` (case-insensitive), THE Title_Builder SHALL omit the Color segment; OTHERWISE WHEN a top or side icing colour resolves to a non-empty colour name (hex codes mapped via the shared `hexToName` palette), THE Title_Builder SHALL include that title-cased colour name as the Color segment.
7. THE Title_Builder SHALL omit any segment whose every word already appears (case-insensitive) in the portion of the title assembled before it (no duplicate words across segments).
8. WHEN the assembled title exceeds `budget` code points, THE Title_Builder SHALL reduce length by dropping segments in the order Detail → Color → Type → Occasion, and only if still over budget SHALL it word-truncate Theme_Segment while preserving the trailing ` Cake`; THE final output length SHALL be ≤ `budget` code points.
9. THE Title_Builder SHALL NEVER include a numeric internal design code, the substring ` with Price`, or a price value anywhere in its output.
10. WHEN the rendered `<title>` is composed as `buildCakeTitle(...) + Title_Suffix`, THE total length SHALL be ≤ 60 code points for every design whose `buildCakeTitle` output is ≤ Title_Budget (which criterion 8 guarantees).
11. THE Title_Builder output SHALL be deterministic: identical `input` (and `budget`) SHALL always yield an identical string.
12. WHEN `generateMetadata` builds the PDP `<title>`, THE Metadata_Builder SHALL use the stored Reconstructed_Title (`design.seo_title`) as the title body when present and non-empty, and SHALL NOT append any Price_Segment to the title.

### Requirement 7: One-Time Full Title Reconstruction Backfill

**User Story:** As a Genie.ph SEO operator, I want every stored `seo_title` reconstructed from the design's own attributes — fixing both the ID-leaked titles and the generic/clean-but-uninformative ones — so that all ~10,591 PDPs render accurate, distinct, customer-facing titles.

#### Acceptance Criteria

1. THE backfill SHALL provide a preview mode that, when invoked, returns for each affected row the `slug`, the current `seo_title`, and the proposed Reconstructed_Title computed by the Title_Builder — WITHOUT writing to any row — AND SHALL be reviewable as an exported artifact (e.g. CSV/JSON) before any apply.
2. THE backfill SHALL compute each row's new value using the SAME Title_Builder logic as the write path (R10), reading only non-PII structured attributes (`keywords`, `analysis_json.cakeType`, `analysis_json.icing_design.colors`, `analysis_json.icing_design.color_type`, `tags`, and hero-topper descriptions from `analysis_json.main_toppers`).
3. THE backfill apply mode SHALL update ALL rows in `cakegenie_analysis_cache` (both ID-leaked rows per ID_Leak_Substring AND clean rows) whose computed Reconstructed_Title differs from the current `seo_title`; rows whose computed title equals the current value SHALL be left unchanged.
4. THE backfill SHALL execute apply mode inside a single transaction that performs both the Backup_Store writes and the `seo_title` UPDATEs, such that any failure rolls back all changes atomically.
5. THE backfill SHALL write the original `seo_title` value of every row it modifies to Backup_Store (recording at minimum `slug`, `seo_title_before`, `backed_up_at`, and a `migration_id`) before that row's UPDATE executes; Backup_Store entries SHALL be retained for at least 30 days after apply.
6. THE backfill SHALL NOT alter any column other than `seo_title` (and the Backup_Store table).
7. WHEN the backfill completes in apply mode AND ISR_Window has elapsed AND at least one HTTP GET has reached the Next.js server for each affected slug after that elapse, THE rendered HTML of every affected PDP SHALL contain neither a `<title>` nor an `<h1>` whose text matches the regular expression ` - \d{2,}( |$|<)` (no leaked numeric ID), AND the `<title>` SHALL equal `Reconstructed_Title + Title_Suffix`.
8. THE backfill SHALL require an explicit confirmation flag (such as `--confirm` or `apply: true`) to enter apply mode; absent that flag THE backfill SHALL execute preview mode by default, and SHALL NOT enter apply mode based on environment variables alone.
9. THE backfill SHALL provide a restore operation that writes `seo_title_before` back to the corresponding row, invocable per-row OR for all rows backed up under a single `migration_id`.
10. WHEN the backfill is run a second time after an apply with no intervening data changes, THE preview SHALL report zero differing rows and apply SHALL update zero rows (idempotence).
...
4. WHEN the Audit_Pipeline is executed against `https://genie.ph/customizing/kuromi-light-purple-1-tier-cake-e3c3` AFTER Requirement 7 apply mode has run AND ISR_Window has elapsed AND at least one HTTP request has reached the server post-elapse, THE parsed snapshot `title` field SHALL NOT contain the substring ` - 1002`, SHALL NOT match ` - \d{2,}( |$|<)`, SHALL contain the substring `Kuromi-Inspired`, AND THE first element of the parsed snapshot `h1` array SHALL NOT contain the substring ` - 1002`.
...
8. WHEN the Audit_Pipeline is executed against every URL in Reference_PDP_Set after Requirement 7 apply mode has run, THE parsed snapshot `title` for every URL SHALL equal `Reconstructed_Title + ' | Genie.ph'` AND its length SHALL be ≤ 60 code points.
...
### Requirement 10: Write-Path Title Generation for New Designs

**User Story:** As a Genie.ph operator, I want newly fingerprinted cake designs to ship with a correct reconstructed title automatically, so that the title-quality fix does not silently regress as new PDPs are created after the backfill.

#### Acceptance Criteria

1. WHEN the cache-write path in `src/services/supabaseService.ts` persists a row to `cakegenie_analysis_cache` (the `seo_title` field of the upsert payload), THE write path SHALL set `seo_title` to the Title_Builder output computed from the same design's structured attributes, and SHALL NOT use `analysisResult.seo_title` (the AI-emitted value) as the persisted title.
2. THE write path SHALL pass to the Title_Builder the design's `keyword`, `cakeType` (from `analysisResult.cakeType`), icing top/side colours and `color_type` (from `analysisResult.icing_design`), the generated `tags`, and hero-topper descriptions (from `analysisResult.main_toppers` entries whose `classification` is `hero`).
3. THE write path SHALL NOT read or persist any title text derived from PII_Source_Fields.
4. WHERE `analysisResult.keyword` is empty, THE write path SHALL still produce a valid title via the Title_Builder's `Custom` fallback (R6.3), never an empty string.
5. THE Title_Builder used by the write path SHALL be the SAME exported function used by the R7 backfill, so that a design fingerprinted after the backfill yields the same title the backfill would have produced for identical attributes.
6. THE write path change SHALL NOT alter the existing `slug`, `alt_text`, `seo_description`, `tags`, `price`, or `availability` generation behaviour.
