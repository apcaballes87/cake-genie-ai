/**
 * Backfill `cakegenie_analysis_cache`.`add-on-price` from the current pricing rules.
 * The stored value is rounded down to an add-on-friendly 99 ending:
 * values below 99 become 0, then 99, 199, 299, 399, etc.
 *
 * Usage:
 *   npx tsx scripts/backfill-add-on-prices.ts --dry-run --limit=25
 *   npx tsx scripts/backfill-add-on-prices.ts
 *   npx tsx scripts/backfill-add-on-prices.ts --all
 *   npx tsx scripts/backfill-add-on-prices.ts --show-pricing-warnings
 *
 * By default, the script recalculates rows where `add-on-price` is null or 0.
 * Use --all to overwrite every row that has analysis_json.
 */
...
if (values.help) {
  console.log(`
Backfill cakegenie_analysis_cache.add-on-price from pricing_rules.

Options:
  --dry-run              Calculate and log changes without updating rows.
  --all                  Recalculate all rows with analysis_json.
...
const recalculateAll = Boolean(values.all);
...
function roundAddOnPriceDownToNearest99(price: number) {
...
function mapAnalysisToPricingState(rawAnalysis: unknown): PricingState {
  ...
  return {
    ...
    cakeInfo: {
      type: asString(analysis.cakeType, '1 Tier'),
...
async function loadPricingCalculator() {
  ...
  if (!calculatePriceFromDatabase) {
    throw new Error('Could not load calculatePriceFromDatabase from pricingService.database.ts.');
  }

  return calculatePriceFromDatabase;
}

function shouldProcess(row: AnalysisCacheRow) {
  if (recalculateAll || pHash) return true;
  return row['add-on-price'] === null || row['add-on-price'] === 0;
}

async function fetchBatch(offset: number) {
  let query = supabase
    .from('cakegenie_analysis_cache')
    .select('id, p_hash, analysis_json, add-on-price')
...
async function main() {
  const calculatePriceFromDatabase = await loadPricingCalculator();

  console.log(
    `Starting add-on price backfill${dryRun ? ' (dry run)' : ''}${recalculateAll ? ' for all rows' : ' for empty/zero rows'}...`
  );
  ...
  for (let offset = 0; ; offset += batchSize) {
    ...
    for (const row of rows) {
      if (limit !== null && processed >= limit) {
        ...
        printSummary({ fetched, processed, updated, unchanged, skipped, errors });
        return;
      }
      ...
      try {
        ...
        const { addOnPricing } = await calculatePriceFromDatabase(pricingState, merchantId);
        const nextAddOnPrice = roundAddOnPriceDownToNearest99(addOnPricing.addOnPrice);

        if (row['add-on-price'] === nextAddOnPrice) {
          unchanged++;
          continue;
        }
        ...
        if (dryRun) {
          console.log(`[dry-run] ${label}: ${row['add-on-price']} -> ${nextAddOnPrice}`);
          updated++;
          continue;
        }
        ...
        const { error: updateError } = await supabase
          .from('cakegenie_analysis_cache')
          .update({ 'add-on-price': nextAddOnPrice })
          .eq('id', row.id);
...
main().catch((error) => {
  console.error('Unexpected backfill failure:', error instanceof Error ? error.message : error);
  process.exit(1);
});
