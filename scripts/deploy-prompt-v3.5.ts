...
async function deployPrompt() {
    ...
    const version = '3.5';
    const description = 'v3.5 - Enhanced Flower Cluster Detection. Adds subtype "flower_cluster" for groups of 3+ 3D flowers.';
    ...
    if (fetchError) {
        ...
        // If no active prompt, we might need to find ANY prompt or handle differently
        process.exit(1);
    }
    ...
    if (newPrompt) {
        ...
        // If we can't see the return value due to RLS, just log success
