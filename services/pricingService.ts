...
import { HybridAnalysisResult, MainTopperUI, SupportElementUI, CakeMessageUI, IcingDesignUI, AddOnPricing, MainTopperType, SupportElementType, Size } from '../types';

// --- Pricing Rules ---

// Nuanced pricing for individual 3D toppers based on complexity and size
...
// Tiered pricing for groups of small, repetitive items based on quantity
...
const getPanelPrice = (coverage: 'light' | 'medium' | 'heavy' | 'none'): number => {
    return { heavy: 300, medium: 200, light: 100, none: 0 }[coverage];
};

const getChocolatePrice = (coverage: 'light' | 'medium' | 'heavy' | 'none'): number => {
    return { heavy: 300, medium: 200, light: 100, none: 0 }[coverage];
};
...
export const calculatePrice = (
    ...
    Object.values(topperGroups).forEach(group => {
        ...
        const isNumberTopper = /\b\d+\b|number/i.test(first.description);
        if (!isNumberTopper) {
            switch (first.type) {
                case 'edible_3d':
                case 'figurine':
