...
import { getLatestImageStudioBatch, reconcileImageStudioBatch, submitNextImageStudioBatch } from '@/lib/admin/imageStudioBatch';
...
export async function GET(req: NextRequest) {
  ...
  try { return NextResponse.json({ run: await getLatestImageStudioBatch() }); }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ run: await submitNextImageStudioBatch(body.limit ?? 1000) });
  } catch (error) {
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(await reconcileImageStudioBatch(body.runId));
  } catch (error) {
