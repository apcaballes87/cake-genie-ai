...
import { MagicSparkleIcon, ErrorIcon, ImageIcon, ResetIcon, SearchIcon, SaveIcon, CartIcon, BackIcon, ReportIcon, UserCircleIcon, LogOutIcon, Loader2, MapPinIcon, PackageIcon } from '../../components/icons';
import { HybridAnalysisResult, MainTopperUI, SupportElementUI, CakeMessageUI, IcingDesignUI, CakeInfoUI, BasePriceInfo } from '../../types';
...
function calculateAvailability(
  ...
  icingDesignType: string | null
): AvailabilityInfo {
  // Check if design has gumpaste accessories (hand-made figures)
  const gumpasteAccessories = [
    'gumpaste figures',
    'gumpaste flowers',
    'hand-made figures',
    'fondant figures',
  ...
  const hasGumpaste = selectedAccessories.some(accessory =>
    gumpasteAccessories.some(gumpaste =>
      accessory.toLowerCase().includes(gumpaste.toLowerCase())
    )
  );

  // Check for printout toppers
  const hasPrintout = selectedAccessories.some(accessory =>
    accessory.toLowerCase().includes('printout') ||
    accessory.toLowerCase().includes('edible image') ||
    accessory.toLowerCase().includes('photo topper')
  );

  // Check if design is simple icing only (not doodle)
  const isSimpleIcingOnly = 
    icingDesignType === 'simple' || 
    icingDesignType === 'minimalist' ||
    (selectedAccessories.length === 0 && !hasPrintout);

  const isDoodleIcing = 
    icingDesignType === 'doodle' || 
    selectedAccessories.some(a => a.toLowerCase().includes('doodle'));

  // LOGIC FOR AVAILABILITY
  
  // Rush: Minimalist cakes with simple icing only (NOT doodle)
  if (isSimpleIcingOnly && !isDoodleIcing && !hasGumpaste && !hasPrintout) {
    return {
      type: 'rush',
  ...
  // Same-Day: No gumpaste, just printouts and icing
  if (!hasGumpaste && (hasPrintout || !isDoodleIcing)) {
    return {
      type: 'same-day',
  ...
  // Normal: Everything else (complex designs)
  return {
    type: 'normal',
...
const CustomizingPage: React.FC<CustomizingPageProps> = ({
    onClose,
    searchInput, setSearchInput, onKeyDown, onSearch,
    setAppState, itemCount, isAuthenticated, isAccountMenuOpen, setIsAccountMenuOpen, accountMenuRef, user, onSignOut,
    onOpenReportModal, editedImage, isLoading, isUpdatingDesign, isReporting, reportStatus, mainImageContainerRef,
  ...
  useEffect(() => {
    if (analysisResult) {
      const selectedAccessories = [
        ...mainToppers.filter(t => t.isEnabled).map(t => t.description),
        ...supportElements.filter(s => s.isEnabled).map(s => s.description)
      ];
  
      // Infer icing design type for more accurate availability
      let icingDesignType: 'simple' | 'minimalist' | 'doodle' | null = null;
      if (icingDesign) {
          const hasDoodleInAccessories = selectedAccessories.some(a => a.toLowerCase().includes('doodle'));
          if (hasDoodleInAccessories) {
              icingDesignType = 'doodle';
          } else if (icingDesign.color_type === 'single' && !icingDesign.drip && !icingDesign.border_top && !icingDesign.border_base && selectedAccessories.length <= 1) {
              icingDesignType = 'minimalist';
          } else if (icingDesign.color_type === 'single') {
              icingDesignType = 'simple';
          }
      }
  ...
  }, [mainToppers, supportElements, icingDesign, analysisResult]);
           ...
           <div className="relative flex-grow">
               ...
               />
                <div className="absolute inset-y-0 right-0 flex items-center pr-2">
                   <button type="button" onClick={onSearch} className="p-3 text-slate-500 hover:text-purple-600 rounded-full hover:bg-purple-100 transition-colors" aria-label="Search">
                       <SearchIcon />
                   </button>
               </div>
           </div>
       ...
       <div ref={mainImageContainerRef} className="w-full max-w-4xl aspect-[7/6] bg-white/70 backdrop-blur-lg rounded-2xl shadow-lg border border-slate-200 flex flex-col">
           ...
           <div className="relative flex-grow flex items-center justify-center p-2 pt-0 min-h-0">
               ...
               {(originalImagePreview) && (
                 ...
                 >
                   <img key={activeTab} src={activeTab === 'customized' ? (editedImage || originalImagePreview) : originalImagePreview} alt={activeTab === 'customized' && editedImage ? "Edited Cake" : "Original Cake"} className="w-full h-full object-cover rounded-lg"/>
                 </button>
               )}
           </div>
       </div>
