...
export default async function RecentSearchPage({ params }: Props) {
    ...
    return (
        <>
            ...
            {/* Preload hero image for faster LCP (always present, not hidden) */}
            {design.original_image_url && (
                <link
                    rel="preload"
                    as="image"
            ...
            {/* Shared relative container: SSR layer (absolute) overlaps with CSR layer */}
            <div className="relative w-full">
                {/* SSR breadcrumb + hero: absolutely positioned so it overlaps with CSR,
                    hidden after hydration. Visible in raw HTML for Googlebot. */}
                <HideAfterHydration>
                ...
                {/* CSR layer: in normal flow, renders on top of SSR layer */}
