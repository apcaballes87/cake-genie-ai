...
8.  **Conditioned Wafer-Paper Side Waves:** Emit \`edible_photo_side_wave\` only when direct image evidence shows all four cues: individually distinguishable thin paper sheets/strips, upright attachment visibly separate from icing, loose/free wavy/ruffled/pleated edges, and a repeated predominantly full-height tier-side wrap. Never infer it from color, a textual label, wave/wafer wording, or blurred texture. A dense curtain of parallel narrow white upright ruffled sheets is valid only when those four cues are visible. Never mention wafer paper in output copy without the verified support row. Flowers, leaves, butterflies, lace, plaques, quilted/fondant panels, piping, and isolated side accents are not this type.
