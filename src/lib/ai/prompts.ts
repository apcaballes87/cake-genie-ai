...
- Do not estimate or emit \`size\` for \`main_toppers\` or \`support_elements\`. The application computes it locally from each representative normalized \`size_line\` and the TOP TIER cake diameter measurement line; any model-provided size is ignored.
- **Representative-size-line precedence:** For every \`main_toppers\` or \`support_elements\` row with \`quantity >= 2\`, \`size_line\` must measure exactly one clearly visible physical representative unit—not the full collection, cluster, arrangement, set, or group. This overrides generic grouping, composition-span, combined-coverage, whole-region, and whole-set language whenever quantity is 2 or more. A composition or coverage line is allowed only for an intentionally region/composition-priced row with \`quantity: 1\`; if one representative unit cannot be measured reliably, omit the repeated row.
- Select the \`size_line\` primary dimension by type: visible height for 3D figures, toys, crowns, figurines, and candles; the larger visible height/width span along the item's dominant physical axis for flat toppers; bloom or sphere diameter for flowers and balls; and the longest relevant visible span along one dominant edge or axis for flat artwork, logos, panels, and other flat items. Put endpoints on opposite boundaries of that same dimension and keep the segment within the representative item. Use a vertical or horizontal line by default; allow a slant only when the item or its true primary axis is visibly rotated or perspective-skewed. Never use a corner-to-corner diagonal, a line across empty space, or a slant merely to make the line longer. Fixed-size overrides remain application-owned: sprinkles and thin fabric ribbon bows are small; wafer-paper side waves, edible photo coverage, and satin ribbon are large.
- In fresh line-ratio mode, emit \`size_line\` instead of priced-element \`bbox\`; message bboxes remain optional and are not used for sizing.
- For eligible variable-height single-body cake types (\`1 Tier\`, \`1 Tier Fondant\`, \`Square\`, \`Rectangle\`, \`Square Fondant\`, and \`Rectangle Fondant\`), the application also computes the final \`cakeThickness\` locally from the TOP TIER measurement-line aspect ratio; any model-provided value is provisional and ignored. If the raw diameter-to-height ratio is greater than 2.0, the application uses an effective 2:1 ratio and 3 in height for local calculations only, without moving or rewriting either stored measurement line endpoint. Fixed-height and multi-tier cake types keep their existing allowed-thickness rules.
- Accepted output must still contain one canonical \`cakeThickness\` value valid for its \`cakeType\`; local post-processing supplies or replaces it before final validation.
- The \`cake_measurements.diameter\` and \`cake_measurements.height\` lines must measure the TOP TIER used as the local sizing reference, not the combined multi-tier stack.
- For the diameter, measure one circular/elliptical cake cross-section from opposing left and right edges. Prefer the visible top ellipse; if hidden, use opposing left/right cake-wall edges at one level. Never choose an arbitrary diagonal across the front wall.
- For the height, use the lower/closer arc of the top ellipse—the front rim where the top surface transitions into the front wall—not the highest pixel, rear/back arc, or exposed top surface. End on the near/front lower rim of the cake wall.
