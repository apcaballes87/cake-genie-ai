...
export function buildImageStudioPrompt(brandLabel: string = 'genie.ph'): string {
  return [
    ...
    'HIGHEST PRIORITY — if the source contains a whole cake alongside cupcakes, treat the complete group as one indivisible composite cake set, never as a cupcake-only image. Preserve every visible cake and cupcake exactly: their item count, size, shape, design, relative positions, board, box, and cupcake holder. In particular, a 4-inch Bento + 5 Cupcakes set must remain exactly one 4-inch bento cake plus exactly five cupcakes; never turn it into all cupcakes, never add cupcakes to make 12, and never remove any part of the set.',
    'For every composite cake set (a whole cake alongside cupcakes), change only the background and use natural studio lighting/contact shadows. Do not crop, reframe, restage, rearrange, replace, remove, add, resize, or otherwise alter the complete cake-and-cupcake set. This rule overrides every later instruction about cupcake layouts, hero-subject selection, boards, boxes, holders, or restaging.',
    'If — and only if — the source contains cupcakes with no whole cake, remove the box and cupcake holder, just show a total of 12 pieces of cupcakes (3 rows, 4 columns) neatly together on the surface and retain the icing and design.',
    ...
    'For screenshots with multiple unrelated cakes or items, keep only the most prominent hero cake, ideally the largest or most centered cake, and do not include secondary cakes. A cake-and-cupcake composite set is one product subject, not unrelated items: preserve its entire set exactly as described above.',
    ...
    'For composite cake sets, background replacement is intentional and must leave the entire product set unchanged. For all other sources, Do NOT do a simple background replacement. The final result must read as a fresh ecommerce product photoshoot, not as an edited screenshot, listing capture, collage, composite, or app card.',
    'STITCHED / COMPOSITE IMAGE EDGE CASE — If the source image appears to be two or more different cake or cupcake photos stitched or composited together (evidenced by mismatched lighting, color temperature, frosting style, decoration style, background, or a visible seam/hard cut between sections), treat it as a single design reference, not as separate images. Reconstruct the entire composition as one cohesive product on the seamless light pastel purple cyclorama. For stitched cake tiers: smooth the transitions between tiers so the lighting, shadows, frosting texture, and color grading are consistent across all tiers; blend or repaint the seam area so there is no visible cut line; preserve each tier\'s original design, decorations, toppers, and character figures faithfully; and restage the unified cake as a single hero product shot with natural contact shadows and consistent studio lighting across the full height of the cake. For stitched cupcake sets: unify the lighting, frosting colors, and decoration style across all cupcakes so they look like one batch; blend any visible seams or hard cuts between sections; arrange the full cupcake set neatly together on the surface; and restage as a single cohesive product shot with consistent shadows and studio lighting. In both cases, the goal is to make the stitched image look like a freshly photographed, unified bakery product — not a composite of separate photos.',
    ...
    'If the cake contains any text, message, writing, edible print, piped lettering, topper inscription, or label in a foreign (non-English) language, translate that text into English while preserving the original meaning, intent, sentiment, approximate length, and visual style (script, color, position, and surrounding decorations). English text on the source cake should be preserved as-is.',
...
export function buildImageStudioSystemInstruction(): string {
  return [
    ...
    'Convert cake and cupcake references into polished standalone catalog photos.',
    'A source that contains a whole cake alongside cupcakes is one indivisible composite product set. Preserve the full set exactly, including the number of items, the whole cake, every cupcake, their relative placement, boards, boxes, and holders. A 4-inch Bento + 5 Cupcakes set must remain one 4-inch bento cake plus exactly five cupcakes, never a cupcake-only 12-piece layout.',
    'For composite cake sets, change only the background and natural studio lighting/contact shadows. Do not crop, reframe, restage, rearrange, add, remove, replace, resize, or otherwise alter the product set. This is higher priority than any cupcake-only, hero-subject, board, box, holder, or restaging instruction.',
    'When a non-composite source is a screenshot, collage, social post, or marketplace capture, treat it only as design reference for the cake itself or the cupcakes.',
    'For non-composite screenshot-like inputs, use a cutout-and-restage workflow: extract only the cake or cupcake subject, discard the entire original screenshot scene, then rebuild the image as a new studio photograph.',
    'For non-composite screenshots, the screenshot is disposable source material; only the cake design should survive into the final image (or cupcake design for cupcakes).',
    'For non-composite sources, the final output must look like a newly photographed studio product image, never like a screenshot, listing tile, collage, or simple background swap.',
    'For non-composite screenshots, removing screenshot artifacts and restaging the cake or cupcakes as a premium product photo is higher priority than preserving the original screenshot scene.',
    'Preserve the cake or cupcake design faithfully, but recreate the surrounding scene, lighting, and composition as needed to achieve a realistic product photoshoot result, except that composite cake sets must retain their original complete product composition.',
    'If the source appears to be two or more different cake or cupcake images stitched together (mismatched lighting, frosting style, background, or a visible seam), reconstruct it as one cohesive unified product: smooth transitions, blend seams, unify lighting and color grading across all sections, and restage the full composition as a single hero product shot on the light pastel purple cyclorama.',
  ].join(' ');
}
