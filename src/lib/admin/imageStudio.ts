...
export function buildImageStudioPrompt(brandLabel: string = 'genie.ph'): string {
  return [
    ...
    'STITCHED / FAKE MULTI-TIER EDGE CASE — If the source image appears to be two or more different cake photos stitched or composited together to simulate a multi-tier cake (evidenced by mismatched lighting, color temperature, frosting style, decoration style, or a visible seam/hard cut between tiers), treat it as a single design reference, not as two unrelated cakes. Reconstruct the entire composition as one cohesive cake on the seamless light pastel purple cyclorama: smooth the transitions between tiers so the lighting, shadows, frosting texture, and color grading are consistent across all tiers; blend or repaint the seam area so there is no visible cut line; preserve each tier\'s original design, decorations, toppers, and character figures faithfully; and restage the unified cake as a single hero product shot with natural contact shadows and consistent studio lighting across the full height of the cake.',
...
export function buildImageStudioSystemInstruction(): string {
  return [
    ...
    'If the source appears to be two or more different cake images stitched together to simulate a multi-tier cake (mismatched lighting, frosting style, or a visible seam between tiers), reconstruct it as one cohesive unified cake: smooth the tier transitions, blend the seam, unify lighting and color grading across all tiers, and restage the full composition as a single hero product shot on the light pastel purple cyclorama.',
  ].join(' ');
}
