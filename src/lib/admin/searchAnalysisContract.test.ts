...
describe('search analysis contract', () => {
  ...
  it('drops the Floral Lily photo-backdrop flowers without mutating the historical-shaped input', () => {
    const fixture = JSON.parse(readFileSync(
      join(process.cwd(), 'src/services/prompts/fixtures/floral-lily-ivory-1-tier-cake-6e69.json'),
      'utf8',
    )) as { forbidden_scene_items: Array<Record<string, unknown>> };
    const { role: _role, ...backgroundFloralCluster } = fixture.forbidden_scene_items[0];
    const generated = validAnalysis({
      main_toppers: [{
        type: 'edible_flowers',
        material: 'edible_fondant',
        group_id: 'top_pink_white_lilies',
        classification: 'hero',
        size: 'large',
        quantity: 3,
        description: 'large pink and white edible lily flowers on top of the cake',
      }],
      support_elements: [
        backgroundFloralCluster,
        {
          type: 'edible_flowers',
          material: 'edible_fondant',
          group_id: 'side_pink_white_lily',
          color: '#FFC0CB',
          size: 'large',
          quantity: 1,
          description: 'large pink and white edible lily flower attached to cake side',
        },
      ],
    });
    ...
    expect((generated.support_elements as Array<Record<string, unknown>>)).toEqual(expect.arrayContaining([
  ...
  it.each([
    ['support flower', 'support_elements', {
      type: 'edible_flowers',
      material: 'edible_fondant',
    ...
    ['support balloon prop', 'support_elements', {
      type: 'plastic_ball_regular',
      material: 'plastic',
      group_id: 'scene_balloon_prop',
      color: '#87CEEB',
      size: 'medium',
      quantity: 2,
      description: 'blue plastic balloon photo prop in the scene',
    }],
    ['main toy prop', 'main_toppers', {
      type: 'toy',
      material: 'plastic',
      group_id: 'scene_toy_prop',
      classification: 'hero',
      size: 'medium',
      quantity: 1,
      description: 'plastic teddy bear behind the cake',
    }],
  ])('drops an explicitly scene-only %s regardless of item type', (_label, role, sceneOnlyItem) => {
    const generated = validAnalysis({
      [role]: [sceneOnlyItem],
    });
  ...
  it('keeps objects attached to the cake and does not treat decorative layering as photo scenery', () => {
    const result = postProcessSearchAnalysisResult(validAnalysis({
      support_elements: [
        {
          type: 'edible_3d_ordinary',
          material: 'edible_fondant',
          group_id: 'fondant_clouds',
          color: '#FFFFFF',
          size: 'small',
          quantity: 2,
          description: 'fondant clouds behind the number candle on the cake top',
        },
        {
          type: 'edible_flowers',
          material: 'edible_fondant',
          group_id: 'attached_floral_background_panel',
          color: '#FFC0CB',
          size: 'medium',
          quantity: 3,
          description: 'pink floral background panel attached to cake side',
        },
      ],
    }), typeEnums);

    expect(result.support_elements).toHaveLength(2);
    expect(result.support_elements).toEqual(expect.arrayContaining([
      expect.objectContaining({ group_id: 'fondant_clouds' }),
      expect.objectContaining({ group_id: 'attached_floral_background_panel' }),
    ]));
  });
