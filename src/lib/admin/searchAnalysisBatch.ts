...
function decodeBatchCustomId(value: string | undefined) {
  ...
  const match = value.match(/^(.*)\|size_schema:(legacy_six_band|three_band|local_bbox_area|local_line_ratio|integrated_bbox_v1|integrated_bbox_v2)$/);
  return match
...
function inferBatchSizeSchema(result: unknown): AnalysisGenerationSizeSchema {
  ...
  if ('analysis' in result && 'geometry' in result) {
    const geometryVersion = (result as { geometry?: { geometry_version?: unknown } }).geometry?.geometry_version;
    return geometryVersion === 'integrated_bbox_v2' ? 'integrated_bbox_v2' : 'integrated_bbox_v1';
  }
      ...
      const result = {
        ...
        analysis_size_schema: resolvedSizeSchema === 'local_line_ratio'
          ...
          : resolvedSizeSchema === 'local_bbox_area'
            ? ANALYSIS_SIZE_SCHEMA
            : resolvedSizeSchema === 'integrated_bbox_v2'
              ? INTEGRATED_BBOX_V2_ANALYSIS_SIZE_SCHEMA
