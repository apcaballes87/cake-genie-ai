...
export const MultiColorEditor: React.FC<MultiColorEditorProps> = ({ colors, onColorChange }) => {
  ...
  return (
    <div>
      <label className="block text-[10px] font-medium text-slate-600 mb-2">Palette Colors</label>
      {editingIndex !== null ? (
        ...
        <div className="flex items-center gap-4 flex-wrap">
          {colors.map((color, index) => (
            <div key={index} className="flex items-center gap-2">
              <div className="w-5 h-5 rounded-full border border-slate-300" style={{ backgroundColor: color || '#FFFFFF' }}></div>
              <button
                type="button"
