...
export default function App(): React.ReactElement {
  ...
  const { appState, previousAppState, confirmedOrderId, viewingDesignId, setAppState, setConfirmedOrderId } = useAppNavigation();
  ...
  const renderAppState = () => {
    switch(appState) {
        ...
        />;
        case 'cart': return <CartPage items={[...pendingCartItems, ...supabaseCartItems.map(item => ({ id: item.cart_item_id, image: item.customized_image_url, status: 'complete' as 'complete', type: item.cake_type, thickness: item.cake_thickness, size: item.cake_size, totalPrice: item.final_price * item.quantity, details: item.customization_details as CartItemDetails, }))]} isLoading={isCartLoading} onRemoveItem={removeItemOptimistic} onClose={() => setAppState(previousAppState.current || 'customizing')} onContinueShopping={() => setAppState('customizing')} onProceedToCheckout={() => setAppState('checkout')} onAuthRequired={() => setAppState('auth')} />;
        case 'checkout': return <CheckoutPage onBackToCart={() => setAppState('cart')} onOrderPlaced={(orderId) => { setConfirmedOrderId(orderId); setAppState('order_confirmation'); }} setAppState={setAppState} />;
        case 'order_confirmation': return <OrderConfirmationPage orderId={confirmedOrderId!} onContinueShopping={() => setAppState('landing')} onGoToOrders={() => setAppState('orders')} />;
        ...
        case 'shared_design': return viewingDesignId ? <SharedDesignPage designId={viewingDesignId} onStartWithDesign={handleStartWithSharedDesign} onNavigateHome={() => { setAppState('landing'); }} /> : <LoadingSpinner />;
        ...
        default: return <LandingPage onSearch={(q) => { setSearchInput(q); handleSearch(q); }} onUploadClick={() => setIsUploaderOpen(true)} setAppState={setAppState} />;
    }
  }

  const mainContentPadding = useMemo(() => {
    switch (appState) {
        case 'landing': return 'p-4'; case 'customizing': return 'py-8 px-4'; case 'searching': return 'py-8 px-4';
        case 'cart': case 'order_confirmation': case 'auth': case 'shared_design': case 'about': case 'how_to_order': case 'contact': case 'reviews': return 'py-20 px-4';
        case 'addresses': case 'orders': return 'py-12 px-4'; default: return 'py-12 px-4';
    }
  }, [appState]);
  ...
  return (
    <ErrorBoundary>
        <div className="relative min-h-screen">
            ...
            {!authError && <>
                {/* TEMPORARILY HIDDEN - Cart and Profile Icons */}
                ...
                {appState === 'customizing' && <StickyAddToCartBar price={finalPrice} isLoading={isFetchingBasePrice} isAdding={isAddingToCart} error={basePriceError} onAddToCartClick={handleAddToCart} onShareClick={handleShare} isSharing={isSavingDesign} canShare={!!analysisResult} isAnalyzing={isAnalyzing} onSave={handleSave} isSaving={isImageManagementLoading} editedImage={editedImage} />}
                <ShareModal isOpen={isShareModalOpen} onClose={closeShareModal} shareUrl={shareData?.shareUrl || ''} designId={shareData?.designId || ''} imageUrl={editedImage || originalImagePreview || ''} />
            </>}
        </div>
    </ErrorBoundary>
  );
