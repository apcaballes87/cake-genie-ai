    ...
    return new Promise<void>((resolve, reject) => {
      hookImageUpload(
        ...
        (result) => { // on analysis success
          ...
          setIsAnalyzing(false); // Quick analysis is done
          resolve();
        },
        ...
        updateStateWithCoordinates, // onCoordinatesSuccess
        { 
          imageUrl,
        }
      );
    });
  }, [clearAllState, hookImageUpload, setIsAnalyzing, setAnalysisError, setPendingAnalysisData, setAppState, initializeDefaultState, updateStateWithCoordinates]);
  ...
  const handleAddToCart = useCallback(async () => {
    ...
    if (typeof gtag === 'function') {
        gtag('event', 'add_to_cart', {
            ...
            value: finalPrice,
            items: [{
                item_id: `${analysisResult.cakeType}_${cakeInfo.size}`,
    ...
    switch(appState) {
        ...
        case 'customizing': return <CustomizingPage 
            ...
            onTopperImageReplace={async (topperId, file) => { 
                if (!user) { showError("You must be logged in to upload images."); return; }
            ...
            onSave={handleSave} isSaving={isImageManagementLoading} 
