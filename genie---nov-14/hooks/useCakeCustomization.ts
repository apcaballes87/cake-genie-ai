...
export const useCakeCustomization = () => {
    ...
    // FIX: Add `onMainTopperChange` to allow replacing the entire topper array, needed for the Shopify flow.
    ...
    const handleApplyAnalysis = useCallback((analysisData: HybridAnalysisResult) => {
        ...
        if (!dirtyFields.has('mainToppers')) {
            ...
            groupedToppers.forEach((group, groupId) => {
                ...
                const markers = group.map(t => ({
                    id: uuidv4(),
                    x: t.x!,
                    y: t.y!
                }));

                let initialType = template.type;
                const canBePrintout = ['edible_3d_complex', 'edible_3d_ordinary', 'toy', 'figurine', 'edible_photo'].includes(template.type);
                const isCharacterOrLogo = /character|figure|logo|brand/i.test(template.description);
        ...
        if (!dirtyFields.has('supportElements')) {
            ...
            groupedElements.forEach((group, groupId) => {
                ...
                let initialType = template.type;
                if (template.type === 'edible_photo_side') {
    ...
    const updateStateWithCoordinates = useCallback((coordinateData: HybridAnalysisResult) => {
        console.log("Applying coordinates to state...");
    
        const createMarkerPoints = (items: (MainTopper | SupportElement | CakeMessage)[]) => {
            return items.map(item => ({ id: uuidv4(), x: item.x!, y: item.y! }));
        };
    
        setMainToppers(prevToppers => prevToppers.map(uiTopper => {
            const coordGroup = coordinateData.main_toppers.filter(t => t.group_id === uiTopper.group_id);
            if (coordGroup.length > 0) {
                return { ...uiTopper, x: coordGroup[0].x, y: coordGroup[0].y, markers: createMarkerPoints(coordGroup) };
            }
            return uiTopper;
        }));
    
        setSupportElements(prevElements => prevElements.map(uiElement => {
            const coordGroup = coordinateData.support_elements.filter(s => s.group_id === uiElement.group_id);
            if (coordGroup.length > 0) {
                return { ...uiElement, x: coordGroup[0].x, y: coordGroup[0].y, markers: createMarkerPoints(coordGroup) };
            }
            return uiElement;
        }));
        
        setCakeMessages(prevMessages => prevMessages.map(uiMessage => {
            const original = uiMessage.originalMessage;
            if (!original) return uiMessage;
            // Find the corresponding message in the coordinate data by matching original properties
            const coordMsg = coordinateData.cake_messages.find(cm => 
                cm.text === original.text && 
                cm.position === original.position && 
                cm.type === original.type
            );
            if (coordMsg) {
                return { ...uiMessage, x: coordMsg.x, y: coordMsg.y };
            }
            return uiMessage;
        }));
