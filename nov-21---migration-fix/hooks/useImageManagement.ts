...
import { fileToBase64, analyzeCakeFeatures } from '../services/geminiService.lazy';
...
export const useImageManagement = () => {
  ...
  const handleImageUpload = useCallback(async (
    ...
    try {
        ...
        // --- Perceptual Hash Cache Check ---
        const pHash = await generatePerceptualHash(imageSrc);
        const cachedResult = await findSimilarAnalysisByHash(pHash);

        if (cachedResult) {
            onSuccess(cachedResult);
            return; // End flow here for cached items
        }

        // --- If not cached, proceed with normal analysis ---
        const analysisResult = await analyzeCakeFeatures(imageData.data, imageData.mimeType);
        onSuccess(analysisResult);
        
        // Fire-and-forget cache write
        cacheAnalysisResult(pHash, analysisResult, options?.imageUrl);
