                                        ...
                                        <details className="mt-3">
                                            ...
                                            <div className="mt-2 pl-2 border-l-2 border-slate-200 space-y-1.5 text-xs text-slate-500">
                                                ...
                                                {Object.entries(details.icingDesign.colors).map(([loc, color]) => {
                                                    // FIX: Add type guard to ensure 'color' is a string before passing to DetailItem,
                                                    // as Supabase JSONB types can be inferred as 'unknown'.
                                                    if (typeof color === 'string') {
                                                        return <DetailItem key={loc} label={`${colorLabelMap[loc] || loc.charAt(0).toUpperCase() + loc.slice(1)} Color`} value={color} />;
                                                    }
                                                    return null;
                                                })}
