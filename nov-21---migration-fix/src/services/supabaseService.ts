...
/**
 * Uploads an image for a report to Supabase Storage.
 * @param base64Data The base64 string of the image.
 * @param type 'original' or 'customized'
 * @returns The public URL of the uploaded image.
 */
export const uploadReportImage = async (base64Data: string, type: 'original' | 'customized'): Promise<string> => {
  try {
    // Decode base64
    ...
    const file = new File([blob], `report-${type}-${uuidv4()}.webp`, { type: mimeString });

    const filePath = `reports/${uuidv4()}-${type}.webp`;

    const { error: uploadError } = await supabase.storage
      .from('cakegenie') // Assuming 'cakegenie' is the bucket
    ...
    console.error(`Error uploading ${type} report image:`, err);
    // Fallback: return empty string or handle error as needed. 
    // Reporting should ideally not fail completely if image upload fails, 
    // but for now we'll throw to let the caller know.
    throw new Error(`Failed to upload ${type} image for report.`);
  }
};
...
/**
 * Compresses and uploads a customization image (e.g., for a printout topper) to Supabase Storage.
 * @param file The image file to upload.
 * @param userId The ID of the user uploading the image.
 * @returns The public URL of the uploaded image.
 */
export async function uploadCustomizationImage(file: File, userId: string): Promise<string> {
  try {
    // Compress image before upload, forcing WebP for efficiency
    const compressedFile = await compressImage(file, {
      maxSizeMB: 1, // Keep it reasonably sized
      maxWidthOrHeight: 1024,
      fileType: 'image/webp',
    });

    const filePath = `customizations/${userId}/${uuidv4()}.webp`;
    ...
    throw new Error("Failed to upload your custom image.");
  }
}
