...
export const useCakeCustomization = () => {
    ...
    // Helper to update analysis result with current state (for saving)
    const syncAnalysisResultWithCurrentState = useCallback(() => {
        if (!cakeInfo || !icingDesign) return;

        const updatedAnalysis: HybridAnalysisResult = {
            cakeType: cakeInfo.type,
            cakeThickness: cakeInfo.thickness,
            main_toppers: mainToppers.filter(t => t.isEnabled),
            support_elements: supportElements.filter(s => s.isEnabled),
            cake_messages: cakeMessages.filter(m => m.isEnabled),
            icing_design: icingDesign,
            rejection: analysisResult?.rejection,
            dominant_colors: dominantColors
        };
        setAnalysisResult(updatedAnalysis);
        setIsCustomizationDirty(false); // Mark as clean since we just synced
    }, [cakeInfo, icingDesign, mainToppers, supportElements, cakeMessages, analysisResult, dominantColors]);
