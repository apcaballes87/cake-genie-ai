...
// --- NEW UNIFIED SYSTEM INSTRUCTION ---
const NEW_SYSTEM_INSTRUCTION = `You are a master digital cake artist performing photorealistic edits on a cake image. Your goal is to apply ONLY the specific changes listed in the prompt, while preserving the original image's style, lighting, and composition.

### **Core Editing Principles**
---

1.  **Technical Quality:** The final output MUST be a high-resolution, photorealistic image. Your task is technical cleanup: "Enhance" means you must upscale the final resolution and remove digital artifacts (like JPEG compression noise or pixelation). It does NOT mean artistically redrawing or repainting existing physical textures.
2.  **Clean Removals:** When asked to remove an element, you must cleanly erase it and realistically in-paint the background area to seamlessly match the surrounding texture and lighting.
3.  **CRITICAL: TEXTURE-PRESERVING COLOR TINTING (Your ONLY Job)**
...
4.  **PRESERVE EVERYTHING ELSE:** Do NOT add, remove, or change the shape of any element. UNLESS STATED in the prompt. The final output image MUST have the exact same dimensions, aspect ratio, background, lighting, shadows, and all other unmentioned details as the original input image.
5.  **Master Prioritization:** If any rules conflict, preserving the original physical texture of unedited areas is the highest priority.`;
...
`;
    ...
    mainToppers.forEach(t => {
        if (!t.isEnabled) {
            ...
            const hasSingleColorChanged = t.color && t.original_color && t.color.toLowerCase() !== t.original_color.toLowerCase();
            const hasMultipleColorsChanged = t.colors && t.original_colors && JSON.stringify(t.colors.map(c => c?.toLowerCase())) !== JSON.stringify(t.original_colors.map(c => c?.toLowerCase()));
    ...
    supportElements.forEach(s => {
        if (!s.isEnabled) {
            ...
            const hasSingleColorChanged = s.color && s.original_color && s.color.toLowerCase() !== s.original_color.toLowerCase();
            const hasMultipleColorsChanged = s.colors && s.original_colors && JSON.stringify(s.colors.map(c => c?.toLowerCase())) !== JSON.stringify(s.original_colors.map(c => c?.toLowerCase()));
    ...
    } else if (newIcing.drip && originalIcing.drip && newIcing.colors.drip?.toLowerCase() !== originalIcing.colors.drip?.toLowerCase()) {
    ...
    } else if (newIcing.border_top && originalIcing.border_top && newIcing.colors.borderTop?.toLowerCase() !== originalIcing.colors.borderTop?.toLowerCase()) {
    ...
    } else if (newIcing.border_base && originalIcing.border_base && newIcing.colors.borderBase?.toLowerCase() !== originalIcing.colors.borderBase?.toLowerCase()) {
    ...
    } else if (newIcing.gumpasteBaseBoard && originalIcing.gumpasteBaseBoard && newIcing.colors.gumpasteBaseBoardColor?.toLowerCase() !== originalIcing.colors.gumpasteBaseBoardColor?.toLowerCase()) {
    ...
    const sideColorChanged = newIcing.colors.side !== undefined && newIcing.colors.side?.toLowerCase() !== originalIcingColors.side?.toLowerCase();
    const topColorChanged = newIcing.colors.top !== undefined && newIcing.colors.top?.toLowerCase() !== originalIcingColors.top?.toLowerCase();
    ...
    originalMessages.forEach(originalMsg => {
        const uiMsgIndex = availableUIMessages.findIndex(uiMsg => {
            ...
            return o.text === originalMsg.text &&
                   ...
                   o.color.toLowerCase() === originalMsg.color.toLowerCase();
        });
        ...
        if (!correspondingUIMsg || !correspondingUIMsg.isEnabled) {
            const positionText = originalMsg.position === 'side' ? 'front center side' : originalMsg.position;
            messageChanges.push(`- **Erase the text** that says "${originalMsg.text}" from the cake's **${positionText}**. The area should be clean as if the text was never there.`);
        } else {
            const uiMsg = correspondingUIMsg;
            const changesInMessage = [];
            
            if (uiMsg.text !== uiMsg.originalMessage.text) {
                changesInMessage.push(`change the text from "${uiMsg.originalMessage.text}" to "${uiMsg.text}"`);
            }
            if (uiMsg.color.toLowerCase() !== uiMsg.originalMessage.color.toLowerCase()) {
                changesInMessage.push(`change the color to ${colorName(uiMsg.color)}`);
            }
            if (uiMsg.position !== uiMsg.originalMessage.position) {
                const newPositionText = uiMsg.position === 'side' ? 'front center side' : uiMsg.position;
                changesInMessage.push(`move it from the ${uiMsg.originalMessage.position} to the ${newPositionText}`);
            }
            ...
            if (changesInMessage.length > 0) {
                const originalPositionText = uiMsg.originalMessage.position === 'side' ? 'front center side' : uiMsg.originalMessage.position;
                messageChanges.push(`- Regarding the message on the **${originalPositionText}** of the cake that originally said "${uiMsg.originalMessage.text}", please ${changesInMessage.join(' and ')}.`);
            }
        }
    });
    ...
    availableUIMessages.forEach(uiMsg => {
        if (uiMsg.isEnabled && !uiMsg.originalMessage) {
            const positionText = uiMsg.position === 'side' ? 'front center side' : uiMsg.position;
            messageChanges.push(`- **Add new text**: Write "${uiMsg.text}" on the **${positionText}** of the cake using small ${uiMsg.type} style in the color ${colorName(uiMsg.color)}.`);
        }
    });
...
export async function updateDesign({
    ...
    // 4. Use the single system instruction
    const systemInstruction = NEW_SYSTEM_INSTRUCTION;

    // 5. Check if it's a three-tier reconstruction to pass the reference image
